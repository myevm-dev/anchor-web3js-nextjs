...
const FEE_TREASURY = new PublicKey("PpokPuQ4zhMkTc8B376acPzAXiVZTaakWjqMaWKfJ9P");
...
async function loadTokenList(): Promise<Record<string, TokenMeta>> {
  ...
  for (const url of sources) {
    try {
      ...
      const list: any[] = Array.isArray(data) ? data : Array.isArray(data?.tokens) ? data.tokens : [];
  ...
  await Promise.all(
    mints.map(async (mint) => {
      try {
        ...
        const container = md && typeof md === "object" ? (("data" in md && md.data) ? md.data : md) : null;
...
export default function RetroPage() {
  ...
  async function handleScan(e: React.FormEvent) {
    ...
    try {
      ...
      const fee = +(((totalLamports * FEE_BPS) / 10_000) / LAMPORTS_PER_SOL).toFixed(6);
  ...
  async function handleClaimOne(r: ClosableAccount) {
    try {
      ...
      if (connectedAddress !== scannedOwner) {
        throw new Error("Connected wallet must match the scanned address to close its token accounts.");
      }

      const conn = connectionFromAdapter ?? new Connection(RPC, { commitment: "processed" });

      const tokenAccount = new PublicKey(r.accountAddress);
      const ix = createCloseAccountInstruction(tokenAccount, publicKey, publicKey);
      const tx = new Transaction().add(ix);

      const sig = await sendTransaction(tx, conn);
      await conn.confirmTransaction(sig, "confirmed");

      // Remove row and recompute stats
      setRows((prev) => prev.filter((x) => x.accountAddress !== r.accountAddress));
      setAccountsToClose((c) => (c ? c - 1 : 0));

      setClaimableSol((s) => {
        const next = Math.max((s ?? 0) - r.reclaimable, 0);
        setFeeSol(+((next * FEE_BPS) / 10_000).toFixed(6));
  ...
  const handleClaimAll = async () => {
    try {
      ...
      const conn = connectionFromAdapter ?? new Connection(RPC, { commitment: "processed" });
      const remaining = [...rows]; // snapshot

      for (let i = 0; i < remaining.length; i += MAX_CLOSES_PER_TX) {
        const batch = remaining.slice(i, i + MAX_CLOSES_PER_TX);
        ...
        for (const r of batch) {
          tx.add(
            createCloseAccountInstruction(
              ...
              publicKey, // destination of reclaimed lamports
              publicKey  // owner / close authority
            )
          );
          batchLamports += r.lamports | 0;
        }

        // 10% protocol fee for this batch (after closes)
        const batchFeeLamports = Math.floor((batchLamports * FEE_BPS) / 10_000);
        ...
        // prune closed rows + update stats
        setRows((prev) =>
          prev.filter((r) => !batch.find((b) => b.accountAddress === r.accountAddress))
        );

        setAccountsToClose((c) => (c ?? 0) - batch.length);

        setClaimableSol((s) => {
          const minus = batchLamports / LAMPORTS_PER_SOL;
          const next = Math.max((s ?? 0) - minus, 0);
          setFeeSol(+((next * FEE_BPS) / 10_000).toFixed(6));
          return +next.toFixed(6);
        });
      }
        ...
        <Card className="p-0 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <p className="mt-1 text-sm text-gray-300">
                    Zero-balance SPL token accounts detected for this address. Each typically refunds ~0.002&nbsp;SOL on close.
                </p>
                </div>

                {/* Close All button on the right */}
                <button
                type="button"
                onClick={handleClaimAll}
                disabled={!canClaim || rows.length === 0}
                className="rounded-xl border border-emerald-400/40 bg-emerald-400/15 px-4 py-2.5 text-emerald-200
                            hover:bg-emerald-400/25 disabled:opacity-50 disabled:cursor-not-allowed"
                          ...
                          <button
                            ...
                            className={`rounded-lg border px-3 py-1.5 text-sm ${
                              canClaim
                                ...
                                : "border-white/20 text-gray-400 opacity-60 cursor-not-allowed"
                            }`}
                          >
                            Claim
                          </button>
    ...
    <>
      <div className="text-center">
        ...
        >
          ...
          <span className={`text-lg leading-none transition-transform ${open ? "rotate-180" : "rotate-0"}`} aria-hidden>▾</span>
        </button>
      </div>
...
function Card({
  ...
  return (
    <div className={`rounded-2xl border bg-white/5 border-[#7B4DFF]/40 shadow-[0_0_0_1px_rgba(123,77,255,0.08)] ${className}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-[#7B4DFF]/40 bg-black/40 px-4 py-4">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="text-2xl sm:text-3xl font-semibold text-white tracking-tight text-right tabular-nums">{value}</div>
      </div>
    </div>
  );
}
